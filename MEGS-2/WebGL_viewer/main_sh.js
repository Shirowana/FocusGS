...
function createWorker(self) {
    ...
    function processPlyBuffer(inputBuffer) {
        ...
        // Same buffer structure as main.js:
        // XYZ - Position (Float32) - 12 bytes
        // XYZ - Scale (Float32) - 12 bytes
        // RGBA - colors (uint8) - 4 bytes
        // IJKL - quaternion/rot (uint8) - 4 bytes
        // Total: 32 bytes per vertex
        buffer = new ArrayBuffer(rowLength * vertexCount);

        // Create separate buffers for SH coefficients
        // For degree=3, we have 48 coefficients per vertex (3 channels * 16 coefficients)
        const shRowLength = 48; // 48 coefficients per vertex
        shBuffer = new ArrayBuffer(shRowLength * 4 * vertexCount); // 4 bytes per coefficient (Float32)
        ...
        for (let j = 0; j < vertexCount; j++) {
            ...
            const position = new Float32Array(buffer, j * rowLength, 3);           // offset 0
            const scales = new Float32Array(buffer, j * rowLength + 12, 3);       // offset 12
            const rgba = new Uint8ClampedArray(
                buffer,
                j * rowLength + 24,                                               // offset 24
                4,
            );
            const rot = new Uint8ClampedArray(
                buffer,
                j * rowLength + 28,                                               // offset 28
                4,
            );
            ...
            // Set RGBA colors (using DC SH terms if available, otherwise use red/green/blue)
            if (types["f_dc_0"]) {
            ...
            if (types["f_dc_0"]) {
                const shOffset = j * shRowLength * 4;
                shDataView.setFloat32(shOffset + 0, attrs.f_dc_0, true);  // R
                shDataView.setFloat32(shOffset + 4, attrs.f_dc_1, true);  // G
                shDataView.setFloat32(shOffset + 8, attrs.f_dc_2, true);  // B
                ...
                for (let i = 0; i < 45; i++) {
                    ...
                    if (i < 15) {
                        shDataView.setFloat32(shOffset + 12 + i * 4, value, true);
                    } else if (i < 30) {
                        shDataView.setFloat32(shOffset + 12 + (i - 15 + 15) * 4, value, true);
                    } else {
                        shDataView.setFloat32(shOffset + 12 + (i - 30 + 30) * 4, value, true);
                    }
                }
            }
        }
...
float computeSHChannel(vec3 dir, int channel, int vertexIndex) {
    ...
    result = result +
        SH_C3[0] * y * (3.0 * xx - yy) * shCoeffs[9] +      // Y_{3,-3}
        SH_C3[1] * xy * z * shCoeffs[10] +                  // Y_{3,-2}
        SH_C3[2] * y * (4.0 * zz - xx - yy) * shCoeffs[11] + // Y_{3,-1}
        SH_C3[3] * z * (2.0 * zz - 3.0 * xx - 3.0 * yy) * shCoeffs[12] + // Y_{3,0}
        SH_C3[4] * x * (4.0 * zz - xx - yy) * shCoeffs[13] + // Y_{3,1}
        SH_C3[5] * z * (xx - yy) * shCoeffs[14] +           // Y_{3,2}
        SH_C3[6] * x * (xx - 3.0 * yy) * shCoeffs[15];      // Y_{3,3}
...
async function main() {
    ...
    const url = new URL(
        params.get("url") || "./bicycle_30000.ply",
        location.href
    );
