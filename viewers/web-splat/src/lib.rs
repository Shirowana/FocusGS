...
#[cfg(target_arch = "wasm32")]
thread_local! {
    static APP_EVENT_PROXY: RefCell<Option<EventLoopProxy<AppEvent>>> = const { RefCell::new(None) };
    static LAST_FPS_VALUE: RefCell<f32> = const { RefCell::new(0.0) };
}
...
impl WindowContext {
    ...
    fn set_scene(&mut self, scene: Scene) {
        ...
        if let Some(camera) = scene.camera(0) {
            self.initial_camera = camera.into();
        }
    ...
    fn reset_to_initial_view(&mut self) {
        self.cancle_animation();
        ...
        self.set_camera(self.initial_camera.clone(), Duration::from_millis(200));
    }
...
pub async fn open_window<R: Read + Seek + Send + Sync + 'static>(
    ...
    #[cfg(target_arch = "wasm32")]
    APP_EVENT_PROXY.with(|proxy| {
        proxy.replace(Some(event_loop.create_proxy()));
    });
    ...
    event_loop.run(move |event,target|
        ...
        Event::WindowEvent {
            ...
            WindowEvent::RedrawRequested => {
                ...
                if request_redraw || redraw_ui{
                    ...
                    #[cfg(target_arch = "wasm32")]
                    LAST_FPS_VALUE.with(|fps| {
                        fps.replace(state.fps);
                    });
        ...
        Event::UserEvent(AppEvent::ResetInitialView) => {
            state.reset_to_initial_view();
            state.window.request_redraw();
        }
        _ => {},
    }).unwrap();
...
#[cfg(target_arch = "wasm32")]
#[wasm_bindgen]
pub fn request_reset_view() {
    APP_EVENT_PROXY.with(|proxy| {
        if let Some(proxy) = proxy.borrow().as_ref() {
            let _ = proxy.send_event(AppEvent::ResetInitialView);
        }
    });
}
...
pub fn get_current_fps() -> f32 {
    LAST_FPS_VALUE.with(|fps| *fps.borrow())
}
